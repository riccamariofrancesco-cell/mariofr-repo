...
def resolve_vavoo_url(original_url):
    """
    Se l'URL contiene 'vavoo', segue i redirect fino all'URL finale.
    Altrimenti ritorna l'URL originale.
    """
    if "vavoo" not in original_url.lower():
        return original_url
    
    try:
        http = urllib3.PoolManager()
        response = http.request('GET', original_url, redirect=0, timeout=urllib3.Timeout(connect=10.0, read=15.0))
        
        # Se c'è un redirect (3xx status)
        if 300 <= response.status < 400 and 'location' in response.headers:
            final_url = response.headers['location']
            logga(f"VAVOO Redirect risolto: {original_url} -> {final_url}")
            return final_url
        
        # Se non c'è redirect, ritorna l'URL originale
        logga(f"VAVOO URL diretto (nessun redirect): {original_url}")
        return original_url
    
    except Exception as e:
        logga(f"Errore nella risoluzione VAVOO: {str(e)}")
        return original_url
...
def run():
    ...
    if params.get('action') == 'play':
        ...
        # RISOLVI REDIRECT VAVOO
        url = resolve_vavoo_url(url)
