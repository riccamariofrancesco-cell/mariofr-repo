...
# VERSIONE 1.0.41-stable - 2026-09-07 - 1.2.252
...
def resolve_sky_channel(par_in):
    ...
    Controlla anche la scadenza del link tramite data["fine"]."""
    ...
    try:
        ...
        data_str = data.get("fine", "EXPIRE")
        logga(f"Sky fine: {data_str}")
        if "EXPIRE" not in data_str:
            try:
                ...
                if data_roma < data_adesso:
                    scad = data_roma.strftime("%d/%m/%Y %H:%M:%S")
                    xbmcgui.Dialog().notification(
                        "Sky Italia", f"Link scaduto: {scad}",
                        xbmcgui.NOTIFICATION_WARNING, 4000)
            except Exception as ex:
                logga(f"Sky expiry parse error: {ex}")
...
def resolve_eurotv(eurotv_url):
    ...
    Se l'URL contiene vavoo.to mostra un dialog per scegliere il dominio,
    poi sostituisce il dominio e procede con la risoluzione.
    Aggiornato a MandraKodi v1.2.252: desktop/electron UA, app v1.3.1,
    fallback TS via lokke.app.
    """
    logga(f"resolve_eurotv: {eurotv_url}")

    # Dialog selezione dominio EUROTV
    if 'vavoo.to' in eurotv_url.lower():
        choice = xbmcgui.Dialog().select(
            "EUROTV - Scegli provider",
            ["vavoo.to", "kool.to", "oha.to", "huhu.to"]
        )
        if choice == -1:
            return eurotv_url  # annullato → usa originale
        chosen_domain = EUROTV_DOMAINS[choice]
        if chosen_domain != 'vavoo.to':
            eurotv_url = re.sub(r'vavoo\.to', chosen_domain, eurotv_url, count=1, flags=re.IGNORECASE)
            logga(f"EUROTV dominio scelto: {chosen_domain} -> {eurotv_url}")

    # Metodo 1: API MediaHubMX con firma addonSig (desktop/electron, v1.3.1)
    try:
        ...
        ping_data = {
            ...
            'package': 'tv.vavoo.app', 'version': '1.3.1', 'process': 'app',
            'firstAppStart': adesso - 86400000, 'lastAppStart': adesso,
        ...
        ping_resp = http_post_json('https://www.vavoo.tv/api/app/ping', ping_data,
                                   headers=ping_headers, timeout=10)
        ...
        if signature:
            res_resp = http_post_json(
                ...
                headers={'user-agent': 'MediaHubMX/2', 'accept': 'application/json',
                         'content-type': 'application/json; charset=utf-8',
                         'content-length': '115', 'accept-encoding': 'gzip',
    ...
    # Metodo 2: TS fallback via lokke.app
    try:
        LOKKE_VEC = ("9frjpxPjxSNilxJPCJ0XGYs6scej3dW/h/VWlnKUiLSG8IP7mfyDU7NirOlld+VtCKGj03Xj"
                     "etfliDMhIev7wcARo+YTU8KPFuVQP9E2DVXzY2BFo1NhE6qEmPfNDnm74eyl/7iFJ0EETm6X"
                     "bYyz8IKBkAqPN/Spp3PZ2ulKg3QBSDxcVN4R5zRn7OsgLJ2CNTuWkd/h451lDCp+TtTuvnAE"
                     "hcQckdsydFhTZCK5IiWrrTIC/d4qDXEd+GtOP4")
        payload = urllib.parse.urlencode({'vec': LOKKE_VEC}).encode('utf-8')
        req_l = urllib.request.Request(
            'https://www.lokke.app/api/box/ping2',
            data=payload,
            headers={'User-Agent': UA_FALLBACK, 'Content-Type': 'application/x-www-form-urlencoded'},
            method='POST')
        with urllib.request.urlopen(req_l, timeout=10) as r:
            lokke_resp = json.loads(r.read().decode('utf-8', errors='replace'))
        ts_signature = lokke_resp.get('response', {}).get('signed')
        if ts_signature:
            new_link = re.sub(r'vavoo-iptv', 'live2', eurotv_url)
            if new_link.endswith('/index.m3u8'):
                new_link = new_link[:-12]
            ts_url = f"{new_link}.ts?n=1&b=5&vavoo_auth={ts_signature}"
            logga(f"eurotv TS fallback: {ts_url}")
            return ts_url
    except Exception as e:
        logga(f"eurotv TS fallback error: {e}")

    # Metodo 3: redirect diretto
    try:
    ...
    try:
        ...
        for item in zappr_data.get('channels', []):
            ...
            if tipo_ch == 'hls':
                ...
                if lic == 'clearkey':
                    url_str = url_str + '|' + (lambda d: ','.join(f'{k}:{v}' for k,v in d.items()) if isinstance(d, dict) else str(d))(item.get('licensedetails', '0000'))
                else:
        ...
        for item in data.get('channels', []):
            ...
            for chExt in item.get('hbbtv', []):
                ...
                if tipo_e == 'hls':
                    ...
                    if lic_e == 'clearkey':
                        url_e = url_e + '|' + (lambda d: ','.join(f'{k}:{v}' for k,v in d.items()) if isinstance(d, dict) else str(d))(item.get('licensedetails', '0000'))
                    else:
                        url_e = url_e + '|0000'
